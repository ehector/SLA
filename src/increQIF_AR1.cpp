...
double obj_eval_ar1(const arma::mat& X, const arma::vec& y, const arma::vec& nobs, const String& family, const arma::vec& beta){
    ...
    double obj_new = as_scalar(gb_new.t() * pinv(g_all_new * g_all_new.t()) * gb_new);
...
//[[Rcpp::export]]
List increQIFflex_ar1(const arma::mat& X, const arma::vec& y, const arma::mat& x_save, const arma::vec& y_save, const arma::vec& nobs, 
                  const String& family, const arma::vec& beta_old, const arma::vec& g_accum, const arma::mat& g_all_accum, 
                  const arma::cube& S_i_accum, const arma::mat& S_accum, const arma::vec& q,
                  const int& maxit, const double& tol){
    ...
    arma::vec q2 = zeros<vec>(2*p);
    arma::mat q_all = zeros<mat>(2*p,n);
    for(int r=0; r<p; r++){
        q2(r) = q(r);
        q2(r+p) = q(r);
        for(int i=0; i<n; i++){
    ...
    while (!stop_flag){
        ...
        for (int i = 0; i < n; i++){
            ...
            vec gi_new = join_cols(Xi.t() * (yi - mui), 
                                   Xi.t() * Ai_half * M1 * Ai_inv_half * (yi-mui) + ui_new12 + q%ui_new21 );
            mat S_i_new = join_cols(Xi.t() * Ai_half * Ai_half * Xi, 
                                    Xi.t() * Ai_half * M1 * Ai_half * Xi + Si_new + (q*q.t())%Si_new.t());
            gb_new += gi_new;
            g_all_new.col(i) = gi_new;
            Sb_new += S_i_new ;
            Sb_i_new.row(i) = S_i_new;
        ...
        g_sum = q2%g_accum + (q2*q.t())%S_accum * (beta_old - beta_new) + gb_new;
        g_all_sum = q_all%g_all_accum + q_all%S_beta_prod + g_all_new;
        
        S_temp = (q2*q.t())%S_accum + Sb_new;
        C_temp = g_all_sum * g_all_sum.t();
